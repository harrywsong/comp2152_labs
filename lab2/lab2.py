...
while True:
    try:
        combatStrength = int(input("Enter your combat Strength (positive integer): "))
        if combatStrength <= 0:
...
while True:
    try:
        mCombatStrength = int(input("Enter the monster's combat Strength (positive integer): "))
        if mCombatStrength <= 0:
            raise ValueError("Monster's Combat Strength must be a positive integer.")
        break
    except ValueError as e:
        print(f"Invalid input: {e}")
...
heroWeapon = weapons[diceRoll - 1]
print("You are using the weapon: {}".format(heroWeapon))
